#!/usr/bin/env python3
"""
Optimized AI Lead Generation Service
Uses the best models for each specific task to maximize results and minimize costs.
"""
...
class OptimizedAILeadGeneration:
    def __init__(self, api_keys: List[str] = None):
        # Default API keys (you can add more)
        self.api_keys = api_keys or [
            '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',  # Current key
        ...
        self.models = {
            'company_research': 'anthropic/claude-3.5-sonnet',  # Best for detailed analysis
            'contact_discovery': 'anthropic/claude-3.5-haiku',   # Fast and cheap
            'lead_enrichment': 'anthropic/claude-3.5-sonnet',    # Detailed analysis
            'industry_suggestions': 'openai/gpt-4o',             # Strategic thinking
            'lead_ideas': 'anthropic/claude-3.5-sonnet'          # Creative tasks
        }
    ...
    def research_companies(self, industry: str, location: str, company_size: str = "medium") -> List[Dict]:
        """Research companies using Claude 3.5 Sonnet for detailed analysis"""
        try:
            # Detailed prompt for company research
            prompt = f"""
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                ...
                "messages": [
                    {"role": "system", "content": "You are an expert business researcher and market analyst. Provide REAL company information, not sample data. Use actual company names and websites. Be specific and actionable."},
    ...
    def discover_contacts(self, company_name: str, company_website: str = None) -> List[Dict]:
        """Discover contacts using Claude 3.5 Haiku for speed and cost efficiency"""
        try:
            prompt = f"""
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                ...
                "max_tokens": 500,  # Fewer tokens for faster response
                "temperature": 0.2
            })
    ...
    def enrich_lead_data(self, lead_data: Dict) -> Dict:
        """Enrich lead data using Claude 3.5 Sonnet for detailed analysis"""
        try:
    ...
    def suggest_industries(self, historical_data: Dict) -> List[Dict]:
        """Suggest industries using GPT-4o for strategic thinking"""
        try:
            prompt = f"""
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                ...
                "messages": [
                    {"role": "system", "content": "You are a sales strategy expert and market analyst. Analyze data and provide actionable industry recommendations with strategic insights."},
                    {"role": "user", "content": prompt}
                ],
                "max_tokens": 700,
                "temperature": 0.4
    ...
    def generate_lead_ideas(self, user_preferences: Dict) -> List[Dict]:
        """Generate lead ideas using Claude 3.5 Sonnet for creativity"""
        try:
            prompt = f"""
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                ...
                "messages": [
                    {"role": "system", "content": "You are a creative marketing strategist and lead generation expert. Generate innovative, actionable lead generation ideas."},
                    {"role": "user", "content": prompt}
                ],
                "max_tokens": 800,
    ...
    # Fallback methods with more realistic data
