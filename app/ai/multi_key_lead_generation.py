#!/usr/bin/env python3
"""
Multi-Key AI Lead Generation Service
Uses multiple API keys as fallbacks for more specific and varied results.
"""
...
class MultiKeyAILeadGeneration:
    ...
    def research_companies(self, industry: str, location: str, company_size: str = "medium") -> List[Dict]:
        """Research companies with multiple API keys for better results"""
        try:
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                ...
                "max_tokens": 800,  # Increased for more detailed results
                "temperature": 0.4
            })
    ...
    def discover_contacts(self, company_name: str, company_website: str = None) -> List[Dict]:
        """Discover contacts with multiple API keys"""
        try:
            prompt = f"""
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                "model": "anthropic/claude-3.5-sonnet",
                "messages": [
                    {"role": "system", "content": "You are an expert at finding business contacts. Provide realistic contact information."},
                    {"role": "user", "content": prompt}
                ],
                "max_tokens": 600,
                "temperature": 0.3
    ...
    def enrich_lead_data(self, lead_data: Dict) -> Dict:
        """Enrich lead data with multiple API keys"""
        try:
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                "model": "anthropic/claude-3.5-sonnet",
                "messages": [
                    {"role": "system", "content": "You are a business intelligence expert. Provide detailed, accurate company information."},
                    {"role": "user", "content": prompt}
                ],
                "max_tokens": 700,
                "temperature": 0.3
    ...
    def suggest_industries(self, historical_data: Dict) -> List[Dict]:
        """Suggest industries with multiple API keys"""
        try:
            prompt = f"""
    ...
    def generate_lead_ideas(self, user_preferences: Dict) -> List[Dict]:
        """Generate lead ideas with multiple API keys"""
        try:
            prompt = f"""
            ...
            response = self._make_request_with_fallbacks("chat/completions", {
                "model": "anthropic/claude-3.5-sonnet",
                "messages": [
                    {"role": "system", "content": "You are a creative marketing strategist. Generate innovative lead generation ideas."},
                    {"role": "user", "content": prompt}
                ],
                "max_tokens": 700,
                "temperature": 0.5
