...
from wtforms import StringField, TextAreaField, SelectField, DateField, SubmitField, PasswordField, BooleanField, FloatField
...
class RegistrationForm(FlaskForm):
    ...
    role = SelectField('Role', choices=[
        ...
        ('Productions Manager', 'Productions Manager'),
        ('HR', 'HR'),
        ('Admin', 'Admin')
...
class ProductionTaskForm(FlaskForm):
    """Form for creating and editing production tasks."""
    title = StringField('Task Title', validators=[DataRequired(), Length(min=3, max=256)])
    ...
    task_type = SelectField('Task Type', choices=[
        ...
        ('landscape_editing', 'Landscape Editing'),
        ('fashion_editing', 'Fashion Editing'),
        ('wedding_editing', 'Wedding Editing'),
        ('commercial_editing', 'Commercial Editing'),
    ...
    # Client information
    client_name = StringField('Client Name', validators=[Optional(), Length(max=128)])
    client_contact = StringField('Client Contact', validators=[Optional(), Length(max=128)])
    client_phone = StringField('Client Phone', validators=[Optional(), Length(max=32)])
    client_email = StringField('Client Email', validators=[Optional(), Email(), Length(max=128)])
    
    # Task details
    estimated_duration_hours = FloatField('Estimated Duration (Hours)', validators=[Optional()])
    project_id = StringField('Project ID', validators=[Optional(), Length(max=64)])
    
    # File management
    create_folders = BooleanField('Create Project Folders', default=True)
    lan_server_path = StringField('LAN Server Path', validators=[Optional(), Length(max=512)])
    dropbox_path = StringField('Dropbox Path', validators=[Optional(), Length(max=512)])
    
    # Task workflow
    workflow_stage = SelectField('Workflow Stage', choices=[
        ('planning', 'Planning'),
    ...
    # Additional settings
    auto_create_folders = BooleanField('Auto-create folders on LAN/Dropbox', default=True)
    notify_assignee = BooleanField('Notify assignee via email', default=True)
    set_reminders = BooleanField('Set due date reminders', default=True)
